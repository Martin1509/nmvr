...
def checkered(self, canvas, line_distance, increment):
    ...
    navigateButton = Button(text='Navigate to goal', command=lambda : _thread.start_new_thread(navigateHandler, (self,)), bg='gray', fg='black', font=('helvetica', 10, 'bold'), highlightbackground=('black'))
...
def moveActorKey(event, self):
    ...
    self.object.actor["realY"] = self.yActor
    self.move = True
    publishMessage(self)
...
def buttonHandler(self):
    ...
    if xy != None:
        self.goal = xy
    if realxy != None:
        self.x = realxy[2] - self.object.actor["realX"]
        self.y = realxy[3] - self.object.actor["realY"] 
        self.object.actor["x"] = realxy[0]
        self.object.actor["y"] = realxy[1]
        self.object.actor["realX"] = realxy[2]
        self.object.actor["realY"] = realxy[3]
        self.move = True
    self.click = True
    publishMessage(self)
